...
class Register {
    ...
    public function insertAdm($desnome, $deslogin, $despassword, $dessession) {
        $conexao = new Sql;
        $passwordok = password_hash($despassword, PASSWORD_BCRYPT);
        $conexao->query("INSERT INTO tb_adm (desnome, deslogin, despassword, dessession) VALUES ('$desnome', '$deslogin', '$passwordok', '$dessession')");
    }
    ...
    public function valilogin($login, $senha) {
        ...
        INNER JOIN tb_logins d ON a.descpf = d.descpf
        WHERE d.deslogin = :deslogin";
    ...
    public function valiLoginAdm($login, $senha) {
        ...
        if (count($results)) {
            $password = $results[0]["despassword"];
            $senhaok = password_hash($senha, PASSWORD_BCRYPT);
            if ($password === $senhaok) {
                return false;
    ...
    public function listAllStudents() {
        $sql = new Sql;
        $query = "SELECT a.idstudent, a.desnome, a.descpf, b.descricao
                    FROM tb_students a
                    INNER JOIN tb_turmas_students c ON a.idstudent = c.idstudent
                    INNER JOIN tb_turmas b ON c.idturma = b.id
                    ORDER BY a.desnome ASC ;";
        $results = $sql->select($query);
        return ($results);
    }
    ...
    public function listStudentsTurma($id) {
        $sql = new Sql;
        $query = "SELECT a.idstudent, a.desnome, a.descpf, b.descricao, d.T, d.C, d.P, d.M
            FROM tb_students a
            INNER JOIN tb_turmas_students c ON a.idstudent = c.idstudent
            INNER JOIN tb_turmas b ON c.idturma = b.id
            INNER JOIN tb_notas d ON a.idstudent = d.idstudent
            WHERE d.idturma = $id
            ORDER BY a.desnome ASC";
        $results = $sql->select($query);
        return ($results);
    ...
    public function studentsTurma($id) {
        $sql = new Sql;
        $query = "SELECT a.idstudent,
            a.desnome,
            a.descpf,
            b.descricao
        FROM tb_students a
    ...
    public function dadosAluno($id) {
        $sql = new Sql;
        $results = $sql->select(
            "SELECT a.idstudent, a.desnome, a.descpf, b.descricao, b.id
            FROM tb_students a
            INNER JOIN tb_turmas_students c ON a.idstudent = c.idstudent
            INNER JOIN tb_turmas b ON c.idturma = b.id
    ...
    public function dadosProfessor($id) {
        $sql = new Sql;
        $results = $sql->select("SELECT a.idturma, a.idprofessor, a.idmateria, b.descodigo, b.desnome, c.descricao, c.turno, c.anoletivo, b.idprof, b.descpf, b.desaddress, b.desnumero FROM  tb_professores b INNER JOIN tb_turmas_professores a ON a.idprofessor = b.idprof INNER JOIN tb_turmas c ON a.idturma = c.id WHERE b.idprof = '$id'");
        return $results;
    }
    ...
    public function editAluno($desnome, $descpf, $idturma, $id) {
        $sql = new Sql;
        //dd($desnome . " = ".$idturma." = " . $descpf . " - " .$id);
        $sql->query("SET FOREIGN_KEY_CHECKS=0");
        $sql->query("UPDATE tb_turmas_students SET idturma = :idturma WHERE idstudent = :id",
            array(
                ":idturma" => $idturma,
                ":id" => $id,
            )
